...
class MultiCNNTEEMWord(object):
    ...
    def accuracy(self,true_y,pre_y,delta=0.8):
        ...
        all_num=len(true_y)
        right_num=0
        pre_y=np.greater(pre_y,np.expand_dims(np.minimum(delta,np.max(pre_y,axis=-1)),axis=-1)).astype(np.int32)
...
if __name__=="__main__":
    ...
    model=MultiCNNTEEMWord(l2_scale=l2_scale)
    
    model.train(qs,labels,nb_epoch=30,val_split=0.2)
    model.save_weigths()
    ...
    tpre2017,tresults2017=write_results(model,inpath=config.seg_test_question2017_path,outpath=config.test_subject_result2017,question_len=20)
